...
| A batch to a file is written all at once, or not at all                       | CLI sketch       | `cmd/ctchargen.buffered`                   | `ctchargen.TestABatchToAFileIsStillWrittenAtOnce`, `ctchargen.TestARefusedBatchWritesNothing` |
| A stream that stops being read stops the batch, naming the member             | CLI sketch       | `cmd/ctchargen.streamed`                   | `ctchargen.TestABatchReportsAStreamThatStoppedBeingRead`                                      |
