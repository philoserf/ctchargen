# Generation record: Unnamed

Seed 7 (go-math-rand-v2-pcg), engine 0.4.0, policy 3.

## characteristics

- (1) roll 2D for each characteristic, in order (p. 4)
- (2) strength: threw 6+1 = 7
- (3) dexterity: threw 5+1 = 6
- (4) endurance: threw 2+2 = 4
- (5) intelligence: threw 5+2 = 7
- (6) education: threw 3+4 = 7
- (7) social_standing: threw 4+3 = 7
- (8) → UPP 764777 (p. 8)

## enlistment

- (9) one enlistment attempt; rejection offers the draft (p. 5)
- (10) service: chose Navy (by policy; options: Navy, Marines, Army, Scouts, Merchants, Other)
- (11) enlistment Navy: threw 2+3 = 5 against 8+: failure
- (12) → rejected by the Navy [from 11]
- (13) submit-to-draft: chose yes (by policy; options: yes, no)
- (14) draft: threw 5 = 5
- (15) → drafted into the Merchants (draft number 5) [from 14]

## term-1

- (16) term 1 begins, age 18
- (17) survival: threw 3+4, DM +2 (intelligence 7+) = 9 against 5+: success
- (18) → draftees are not eligible for commission during their first term (p. 5)
- (19) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (20) skill table service_skills: threw 2 = 2
- (21) → Vacc Suit-1 [from 20]
- (22) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (23) skill table service_skills: threw 6 = 6
- (24) → Jack-of-all-Trades-1 [from 23]
- (25) reenlist-intent: chose yes (by policy; options: yes, no)
- (26) reenlistment: threw 6+1 = 7 against 4+: success
- (27) → reenlists for another term [from 26]

## term-2

- (28) term 2 begins, age 22
- (29) survival: threw 4+3, DM +2 (intelligence 7+) = 9 against 5+: success
- (30) commission-attempt: chose yes (by policy; options: yes, no)
- (31) commission: threw 2+5, DM +1 (intelligence 6+) = 8 against 4+: success
- (32) → commissioned as 4th Officer (rank 1); +1 skill eligibility (pp. 5-6) [from 31]
- (33) promotion-attempt: chose yes (by policy; options: yes, no)
- (34) promotion: threw 5+6 = 11 against 10+: success
- (35) → promoted to 3rd Officer (rank 2); +1 skill eligibility (p. 6) [from 34]
- (36) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (37) skill table service_skills: threw 1 = 1
- (38) → Steward-1 [from 37]
- (39) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (40) skill table service_skills: threw 2 = 2
- (41) → Vacc Suit-2 [from 40]
- (42) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (43) skill table service_skills: threw 5 = 5
- (44) → Electronic-1 [from 43]
- (45) reenlist-intent: chose yes (by policy; options: yes, no)
- (46) reenlistment: threw 4+6 = 10 against 4+: success
- (47) → reenlists for another term [from 46]

## term-3

- (48) term 3 begins, age 26
- (49) survival: threw 5+1, DM +2 (intelligence 7+) = 8 against 5+: success
- (50) promotion-attempt: chose yes (by policy; options: yes, no)
- (51) promotion: threw 3+4 = 7 against 10+: failure
- (52) → promotion denied this term (one attempt per term, p. 6) [from 51]
- (53) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (54) skill table service_skills: threw 5 = 5
- (55) → Electronic-2 [from 54]
- (56) reenlist-intent: chose yes (by policy; options: yes, no)
- (57) reenlistment: threw 6+4 = 10 against 4+: success
- (58) → reenlists for another term [from 57]

## term-4

- (59) term 4 begins, age 30
- (60) survival: threw 2+5, DM +2 (intelligence 7+) = 9 against 5+: success
- (61) promotion-attempt: chose yes (by policy; options: yes, no)
- (62) promotion: threw 2+4 = 6 against 10+: failure
- (63) → promotion denied this term (one attempt per term, p. 6) [from 62]
- (64) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (65) skill table service_skills: threw 1 = 1
- (66) → Steward-2 [from 65]
- (67) reenlist-intent: chose yes (by policy; options: yes, no)
- (68) reenlistment: threw 6+1 = 7 against 4+: success
- (69) → reenlists for another term [from 68]
- (70) → aging: turning 34 (pp. 7, 9; E005)
- (71) aging strength save: threw 2+5 = 7 against 8+: failure
- (72) → -1 strength (7 → 6), aging (p. 9) [from 71]
- (73) aging dexterity save: threw 1+4 = 5 against 7+: failure
- (74) → -1 dexterity (6 → 5), aging (p. 9) [from 73]
- (75) aging endurance save: threw 1+3 = 4 against 8+: failure
- (76) → -1 endurance (4 → 3), aging (p. 9) [from 75]

## term-5

- (77) term 5 begins, age 34
- (78) survival: threw 6+4, DM +2 (intelligence 7+) = 12 against 5+: success
- (79) promotion-attempt: chose yes (by policy; options: yes, no)
- (80) promotion: threw 4+3 = 7 against 10+: failure
- (81) → promotion denied this term (one attempt per term, p. 6) [from 80]
- (82) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (83) skill table service_skills: threw 6 = 6
- (84) → Jack-of-all-Trades-2 [from 83]
- (85) reenlist-intent: chose yes (by policy; options: yes, no)
- (86) reenlistment: threw 6+5 = 11 against 4+: success
- (87) → reenlists for another term [from 86]
- (88) → aging: turning 38 (pp. 7, 9; E005)
- (89) aging strength save: threw 5+5 = 10 against 8+: success
- (90) aging dexterity save: threw 5+6 = 11 against 7+: success
- (91) aging endurance save: threw 4+2 = 6 against 8+: failure
- (92) → -1 endurance (3 → 2), aging (p. 9) [from 91]

## term-6

- (93) term 6 begins, age 38
- (94) survival: threw 2+2, DM +2 (intelligence 7+) = 6 against 5+: success
- (95) promotion-attempt: chose yes (by policy; options: yes, no)
- (96) promotion: threw 2+5 = 7 against 10+: failure
- (97) → promotion denied this term (one attempt per term, p. 6) [from 96]
- (98) skill-table: chose service_skills (by policy; options: personal_development, service_skills, advanced_education)
- (99) skill table service_skills: threw 3 = 3
- (100) → +1 strength (6 → 7), applied immediately (p. 12) [from 99]
- (101) reenlist-intent: chose yes (by policy; options: yes, no)
- (102) reenlistment: threw 1+2 = 3 against 4+: failure
- (103) → reenlistment denied; must leave the service (p. 6) [from 102]
- (104) → aging: turning 42 (pp. 7, 9; E005)
- (105) aging strength save: threw 3+6 = 9 against 8+: success
- (106) aging dexterity save: threw 1+5 = 6 against 7+: failure
- (107) → -1 dexterity (5 → 4), aging (p. 9) [from 106]
- (108) aging endurance save: threw 2+1 = 3 against 8+: failure
- (109) → -1 endurance (2 → 1), aging (p. 9) [from 108]
- (110) → leaves the service after 6 term(s), age 42

## muster-out

- (111) mustering out: 7 rolls (6 terms + 1 for rank), at most 3 on the cash table (pp. 7, 9)
- (112) muster-table: chose cash (by policy; options: benefits, cash)
- (113) cash table: threw 4 = 4
- (114) → CR 20000 cash (Table 2 row 4; p. 9) [from 113]
- (115) muster-table: chose cash (by policy; options: benefits, cash)
- (116) cash table: threw 3 = 3
- (117) → CR 10000 cash (Table 2 row 3; p. 9) [from 116]
- (118) muster-table: chose cash (by policy; options: benefits, cash)
- (119) cash table: threw 3 = 3
- (120) → CR 10000 cash (Table 2 row 3; p. 9) [from 119]
- (121) muster-table: chose benefits (by policy; options: benefits)
- (122) benefits table: threw 6 = 6
- (123) → low passage (CR 1,000, sellable at 90%; pp. 21-22) [from 122]
- (124) muster-table: chose benefits (by policy; options: benefits)
- (125) benefits table: threw 1 = 1
- (126) → low passage (CR 1,000, sellable at 90%; pp. 21-22) [from 125]
- (127) muster-table: chose benefits (by policy; options: benefits)
- (128) benefits table: threw 6 = 6
- (129) → low passage (CR 1,000, sellable at 90%; pp. 21-22) [from 128]
- (130) muster-table: chose benefits (by policy; options: benefits)
- (131) benefits table: threw 1 = 1
- (132) → low passage (CR 1,000, sellable at 90%; pp. 21-22) [from 131]
- (133) → retired after 6 terms: CR 6000 per year retirement pay (pp. 7, 21)
